...
$querytotal = "SELECT COUNT(*) AS jml
            FROM tb_berkas WHERE tb_berkas.id_jenis = 1 AND tb_berkas.id_pengguna = '$id_pengguna' ORDER BY id_berkas ASC ";
...
if (isset($_GET['keyword-video'])) {  
    $sql = "SELECT tb_berkas.id_berkas,tb_berkas.ukuran, tb_berkas.nama, tb_berkas.file, tb_berkas.deskripsi FROM tb_berkas WHERE tb_berkas.id_jenis = 1 AND tb_berkas.id_pengguna = '$id_pengguna' AND tb_berkas.nama LIKE ? ORDER BY id_berkas ASC";
    $keyword = $_GET['keyword-video'];
    $keyword = $keyword . "%";
    ...
    $sql = "SELECT tb_berkas.id_berkas,tb_berkas.ukuran, tb_berkas.nama, tb_berkas.file, tb_berkas.deskripsi FROM tb_berkas WHERE tb_berkas.id_jenis = 1 AND tb_berkas.id_pengguna = '$id_pengguna' ORDER BY id_berkas ASC LIMIT $awalData,$jumlahDataPerhalaman";
    $stmt = $mysqli->prepare($sql);
    $stmt->execute();
    $berkas = $stmt->get_result();
}
    ...
    <div class="col-span-12 lg:col-span-3 xxl:col-span-2">
        ...
        <div class="intro-y box p-5 mt-6">
            <div class="mt-1">
                ...
                <a href="index.php?video" class="flex items-center px-3 py-2 mt-2 rounded-md bg-theme-1 text-white font-medium"> <i class="w-4 h-4 mr-2" data-feather="video"></i> Videos </a>
    ...
    <div class="col-span-12 lg:col-span-9 xxl:col-span-10">
        ...
        <div class="intro-y flex flex-col-reverse sm:flex-row items-center">
            <form class="w-full sm:w-auto relative mr-auto mt-3 sm:mt-0" method="GET" action="">
                ...
                <input id="keyword" type="text" class="input w-full sm:w-64 box px-10 text-gray-700 dark:text-gray-300 placeholder-theme-13" name="keyword-video" placeholder="Search files">
        ...
        <div class="intro-y flex flex-wrap sm:flex-row sm:flex-no-wrap items-center mt-6">
            <ul class="pagination">
                <?php if (!isset($keyword)&&$totalfile>12): ?>
                    ...
                    <li>
                        <a class="pagination__link" href="?video&halaman=<?= $halamanaktif - 1;?>"> <i class="w-4 h-4" data-feather="chevrons-left"></i> </a>
                    </li>
                    <?php endif; ?>
                    <?php for($i=$start_number;$i <= $end_number;$i++): ?>

                        <?php if($i == $halamanaktif): ?> 
                            <li> <a class="pagination__link pagination__link--active" href="?video&halaman=<?= $i; ?>"><?= $i; ?></a> </li>
                        <?php else : ?>
                            <li> <a class="pagination__link" href="?video&halaman=<?= $i; ?>"><?= $i; ?></a> </li>
                        <?php endif;  ?>
                    <?php endfor; ?>
                    <?php if($halamanaktif < $jumlahHalaman): ?>
                        <li>
                            <a class="pagination__link" href="?video&halaman=<?= $halamanaktif + 1; ?>"> <i class="w-4 h-4" data-feather="chevrons-right"></i> </a>
                        </li>
                    <?php endif; ?>
                <?php endif; ?>
            </ul>
