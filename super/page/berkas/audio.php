...
$querytotal = "SELECT COUNT(*) AS jml
            FROM tb_berkas WHERE tb_berkas.id_jenis = 3 AND tb_berkas.id_pengguna = '$id_pengguna' ASC ";
...
if (isset($_GET['keyword-audio'])) {  
    $sql = "SELECT tb_berkas.id_berkas,tb_berkas.ukuran, tb_berkas.nama, tb_berkas.file, tb_berkas.deskripsi FROM tb_berkas WHERE tb_berkas.id_jenis = 3 AND tb_berkas.id_pengguna = '$id_pengguna' AND tb_berkas.nama LIKE ? ORDER BY id_berkas ASC";
    $keyword = $_GET['keyword-audio'];
    $keyword = $keyword . "%";
    ...
    $sql = "SELECT tb_berkas.id_berkas,tb_berkas.ukuran, tb_berkas.nama, tb_berkas.file, tb_berkas.deskripsi FROM tb_berkas WHERE tb_berkas.id_jenis = 3 AND tb_berkas.id_pengguna = '$id_pengguna' ORDER BY id_berkas ASC LIMIT $awalData,$jumlahDataPerhalaman";
    $stmt = $mysqli->prepare($sql);
    $stmt->execute();
    $berkas = $stmt->get_result();
}
...
<div class="grid grid-cols-12 gap-6 mt-8">
    <div class="col-span-12 lg:col-span-3 xxl:col-span-2">
        ...
        <div class="intro-y box p-5 mt-6">
            <div class="mt-1">
                <a href="index.php?gambar" class="flex items-center px-3 py-2 rounded-md "> <i class="w-4 h-4 mr-2" data-feather="image"></i> Gambar </a>
                <a href="index.php?video" class="flex items-center px-3 py-2 mt-2 rounded-md"> <i class="w-4 h-4 mr-2" data-feather="video"></i> Videos </a>
                <a href="index.php?berkas" class="flex items-center px-3 py-2 mt-2 rounded-md"> <i class="w-4 h-4 mr-2" data-feather="file"></i> Dokumen </a>
                <a href="index.php?audio" class="flex items-center px-3 py-2 mt-2 rounded-md  bg-theme-1 text-white font-medium"> <i class="w-4 h-4 mr-2" data-feather="message-square"></i> Audio </a>
                <a href="index.php?other" class="flex items-center px-3 py-2 mt-2 rounded-md"> <i class="w-4 h-4 mr-2" data-feather="box"></i> Lain - lain </a>
            </div>
        </div>
    ...
    <div class="col-span-12 lg:col-span-9 xxl:col-span-10">
        ...
        <div class="intro-y flex flex-col-reverse sm:flex-row items-center">
            <form class="w-full sm:w-auto relative mr-auto mt-3 sm:mt-0" method="GET" action="">
                ...
                <input id="keyword" type="text" class="input w-full sm:w-64 box px-10 text-gray-700 dark:text-gray-300 placeholder-theme-13" name="keyword-audio" placeholder="Search files">
                        ...
                        <div class="absolute top-0 right-0 mr-2 mt-2 dropdown ml-auto">
                            ...
                            <div class="dropdown-box w-40">
                                <div class="dropdown-box__content box dark:bg-dark-1 p-2">
                                    <?php $url = 'berkas/' . $divisib . '/audio/'; ?>
                                    <a id="<?= $row['id_berkas'] ?>" href="javascript:;" data-toggle="modal" data-target="#button-modal-preview" data-ulr="<?= $url ?>"class="flex items-center block p-2 transition duration-300 ease-in-out bg-white dark:bg-dark-1 hover:bg-gray-200 dark:hover:bg-dark-2 rounded-md lihat"> <i data-feather="airplay" class="w-4 h-4 mr-2"></i>Lihat</a>
                                    ...
                                    <a id="<?= $row['id_berkas'] ?>" href="javascript:;" data-toggle="modal" data-target="#delete-modal" data-url="<?= $url; ?><?= $row['file']?>" class="flex items-center block p-2 transition duration-300 ease-in-out bg-white dark:bg-dark-1 hover:bg-gray-200 dark:hover:bg-dark-2 rounded-md hapus"> <i data-feather="trash" class="w-4 h-4 mr-2"></i>Hapus</a>
                                </div>
                            </div>
                        </div>
                    </div>
        ...
        <div class="intro-y flex flex-wrap sm:flex-row sm:flex-no-wrap items-center mt-6">
            <ul class="pagination">
                <?php if (!isset($keyword)&&$totalfile>12): ?>
                    ...
                    <li>
                        <a class="pagination__link" href="?audio&halaman=<?= $halamanaktif - 1;?>"> <i class="w-4 h-4" data-feather="chevrons-left"></i> </a>
                    </li>
                    <?php endif; ?>
                    <?php for($i=$start_number;$i <= $end_number;$i++): ?>

                        <?php if($i == $halamanaktif): ?> 
                            <li> <a class="pagination__link pagination__link--active" href="?audio&halaman=<?= $i; ?>"><?= $i; ?></a> </li>
                        <?php else : ?>
                            <li> <a class="pagination__link" href="?audio&halaman=<?= $i; ?>"><?= $i; ?></a> </li>
                        <?php endif;  ?>
                    <?php endfor; ?>
                    <?php if($halamanaktif < $jumlahHalaman): ?>
                        <li>
                            <a class="pagination__link" href="?audio&halaman=<?= $halamanaktif + 1; ?>"> <i class="w-4 h-4" data-feather="chevrons-right"></i> </a>
                        </li>
                    <?php endif; ?>
                <?php endif; ?>
            </ul>
