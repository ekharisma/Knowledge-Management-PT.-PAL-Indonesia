...
$kodeJenis = $_GET['jenis'];

$querytotal = "SELECT COUNT(*) AS jml
            FROM tb_berkas, tb_pengguna WHERE tb_berkas.id_pengguna = tb_pengguna.id_pengguna AND tb_berkas.kat=1  AND tb_berkas.id_jenis = '$kodeJenis' AND tb_pengguna.id_divisi='$id_divisi'";
...
$sql = "SELECT tb_pengguna.id_pengguna, tb_pengguna.nama_pengguna,tb_pengguna.id_divisi, tb_pengguna.foto, tb_berkas.id_berkas,tb_berkas.nama, tb_berkas.file, tb_berkas.jam, tb_berkas.id_jenis , tb_berkas.tanggal, tb_berkas.view, tb_berkas.download FROM tb_berkas, tb_pengguna WHERE tb_berkas.id_pengguna = tb_pengguna.id_pengguna AND tb_berkas.kat=1  AND tb_berkas.id_jenis = ? AND tb_pengguna.id_divisi='$id_divisi' LIMIT $awalData,$jumlahDataPerhalaman";

$stmt = $mysqli->prepare($sql);
$stmt->bind_param('i', $kodeJenis);
$stmt->execute();
...
<div class="intro-y grid grid-cols-12 gap-6 mt-5">
            ...
            <div class="px-5 pt-3 pb-5 border-t border-gray-200 dark:border-dark-5">
                <div class="w-full flex text-gray-600 text-xs sm:text-sm">
                            <div  class="mr-2"> Views: <span id ="<?= $data['id_berkas'];?>_view_count" class="font-medium"><?php echo $data['view']; ?></span> </div>
                             <?php
                             $totalcomment=0;
                                 $querytotal = "SELECT COUNT(tb_komentar.id_komentar) AS jml
                                            FROM tb_komentar WHERE tb_komentar.id_berkas = '".$data['id_berkas']."'";
    ...
    <!-- BEGIN: Pagination -->
            ...
            <ul class="pagination">
                ...
                <li>
                    <a class="pagination__link" href="?private_detail&jenis=<?= $kodeJenis?>&halaman=<?= $halamanaktif - 1; ?>"> <i class="w-4 h-4" data-feather="chevrons-left"></i> </a>
                </li>
                <?php endif; ?>
                <?php for($i=$start_number;$i <= $end_number;$i++): ?>

                    <?php if($i == $halamanaktif): ?> 
                        <li> <a class="pagination__link pagination__link--active" href="?private_detail&jenis=<?= $kodeJenis?>&halaman=<?= $i; ?>"><?= $i; ?></a> </li>
                    <?php else : ?>
                        <li> <a class="pagination__link" href="?private_detail&jenis=<?= $kodeJenis?>&halaman=<?= $i; ?>"><?= $i; ?></a> </li>
                    <?php endif;  ?>
                <?php endfor; ?>
                <?php if($halamanaktif < $jumlahHalaman): ?>
                    <li>
                        <a class="pagination__link" href="?private_detail&jenis=<?= $kodeJenis?>&halaman=<?= $halamanaktif + 1; ?>"> <i class="w-4 h-4" data-feather="chevrons-right"></i> </a>
                    </li>
                <?php endif; ?>
            </ul>
        </div>
